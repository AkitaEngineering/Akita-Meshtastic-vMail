...
class AkitaVmailApp:
    ...
    def update_ui_state(self):
        ...
        try:
            ...
            # Update status bar metrics (non-intrusive: append to existing status message)
            try:
                pending = self.meshtastic_handler.get_pending_ack_count() if hasattr(self.meshtastic_handler, 'get_pending_ack_count') else 0
                retrans = self.meshtastic_handler.get_total_retransmits() if hasattr(self.meshtastic_handler, 'get_total_retransmits') else 0
                if hasattr(self, 'status_var'):
                    cur = self.status_var.get() or ''
                    prefix = 'Status: '
                    if cur.startswith(prefix):
                        base_msg = cur[len(prefix):].split(' | Pending ACKs:', 1)[0]
                        new_status = f"{prefix}{base_msg} | Pending ACKs: {pending} Retries: {retrans}"
